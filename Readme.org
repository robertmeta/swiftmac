...
SwiftMac uses automated GitHub Actions to build and release new versions.

*** How It Works

When you update the version in =Sources/SwiftMacPackage/main.swift= and push to main:

1. GitHub Actions detects the version change
2. Checks if a release for that version already exists
3. If new, automatically:
   - Builds the release binary
   - Creates a release tarball with all necessary files
   - Creates a Git tag (=v4.2.3=, etc.)
...
1. Update version in =Sources/SwiftMacPackage/main.swift=:
   #+BEGIN_SRC swift
   let version = "4.2.4"  // Change this
   #+END_SRC

2. Commit and push to main:
   #+BEGIN_SRC shell
   git add Sources/SwiftMacPackage/main.swift
   git commit -m "Version 4.2.4: Description of changes"
   git push
   #+END_SRC

3. GitHub Actions will automatically create the release

4. Download link will be available at:
   =https://github.com/robertmeta/swiftmac/releases=
