...
SwiftMac supports stereo channel control for separating different audio types. Useful for distinguishing notifications from main speech on headphones.

*** Runtime Commands

Change audio channels on the fly:

- =tts_set_speech_channel {left|right|both}=: Route speech to left, right, or both channels
- =tts_set_notification_channel {left|right|both}=: Route notifications to left, right, or both channels

*** Common Use Case: Separate Notifications from Speech

Route speech to both channels and notifications to left channel only:

#+BEGIN_SRC emacs-lisp
  ; At startup (or add to your config)
  (setenv "SWIFTMAC_AUDIO_TARGET" "left")  ; Enables notification mode
#+END_SRC

Default behavior:
- Speech: both channels (stereo)
- Notifications: left channel only (when in notification mode)

You can change channels during use with the runtime commands above.
